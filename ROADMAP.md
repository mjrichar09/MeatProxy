...
**E1 and E2 have opened.** The core library, the headless harness and the suite
are in `src/` and `tests/`; the house is walkable, saveable and queryable as
predicates, and the two perception layers are separate structures — covered and
understood are different states, the interpreter can be pulled, and the wifi turn
is a device record. Neither stage has exited — see `src/README.md` for what is
authored and what is a placeholder. **D2 is still deferred** and no verdict is written or assumed;
...
0024–0027 on 2026-09-16, 0028–0034 on 2026-09-18. **ADR 0026 is superseded and
is the only one.** Nothing in `docs/decisions/` is Proposed, so D3 has a complete
and stable input set and every lane may depend on the whole record. Changing one
is itself a decision, routed back through the directory (standing rule 3).
...
| **005** | Too compliant | **T3** | First playtest data on the two discovery numbers: how many players lost by relaxing, and how many found the ending nobody is told about (ADR 0022, ADR 0023) | r/gamedev, YouTube |
