...
less travelled than GDScript, and Godot 4's API churn means a meaningful share
...
And the licence carries no ongoing obligation, which is the same concern B5 is
open on.
...
- It is the better-supported language for the way this project is written.
  GDScript is the better-travelled *Godot* path, and that is the real trade.
